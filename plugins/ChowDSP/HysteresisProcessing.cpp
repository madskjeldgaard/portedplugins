...
inline double HysteresisProcessing::hysteresisFunc(double M, double H,
                                                   double H_d) noexcept {
  Q = (H + alpha * M) / a;
  coth = 1.0 / std::tanh(Q);
...
inline double HysteresisProcessing::hysteresisFuncPrime(double H_d,
                                                        double dMdt) noexcept {
  const double L_prime2 = langevinD2(Q);
  const double M_diff2 = M_s_oa_talpha * L_prime - 1.0;

  const double f1_p = kap1 * ((M_diff2 / f1Denom) +
                              M_diff * alpha * M_diff2 / (f1Denom * f1Denom));
...
inline double HysteresisProcessing::RK2(double H, double H_d) noexcept {
  ...
  const double k2 = T * hysteresisFunc(M_n1 + (k1 / 2.0), (H + H_n1) / 2.0,
                                       (H_d + H_d_n1) / 2.0);
...
inline double HysteresisProcessing::NR(double H, double H_d) noexcept {
  ...
  for (int n = 0; n < numIter; n += 4) {
    ...
    deltaNR =
        (M - M_n1 - Talpha * (dMdt + last_dMdt)) / (1.0 - Talpha * dMdtPrime);
    M -= deltaNR;
