...
class WDFNode : public WDF {
 public:
  ...
  inline void propagateImpedance() override {
    ...
    if (next != nullptr) next->propagateImpedance();
  }
...
class Resistor : public WDFNode {
 public:
   ...
   */
  Resistor(double value) : WDFNode("Resistor"), R_value(value) {
    calcImpedance();
  }
  virtual ~Resistor() {}
...
class PolarityInverter : public WDFNode {
 public:
  ...
  PolarityInverter(WDFNode* port1)
      : WDFNode("Polarity Inverter"), port1(port1) {
...
class YParameter : public WDFNode {
 public:
  ...
  inline void calcImpedance() override {
    denominator =
        y[1][1] + port1->R * y[0][0] * y[1][1] - port1->R * y[0][1] * y[1][0];
...
class ResistiveVoltageSource : public WDFNode {
 public:
  ...
  ResistiveVoltageSource(double value = 1.0e-9)
      : WDFNode("Resistive Voltage"), R_value(value) {
    calcImpedance();
  }
...
class ResistiveCurrentSource : public WDFNode {
 public:
  ...
  ResistiveCurrentSource(double value = 1.0e9)
      : WDFNode("Resistive Current"), R_value(value) {
    calcImpedance();
  }
...
class DiodePair : public WDFNode {
 public:
  ...
  inline double reflected() noexcept override {
    ...
    b = a + 2 * lambda *
                (next->R * Is -
                 Vt * omega4(float(log(next->R * Is / Vt) +
...
class Diode : public WDFNode {
 public:
  ...
  inline double reflected() noexcept override {
    ...
    b = a + 2 * next->R * Is -
        2 * Vt *
            omega4(float(log(next->R * Is / Vt) + (a + next->R * Is) / Vt));
    return b;
  }
